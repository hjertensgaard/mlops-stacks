...
The databricks CLI bundles top level is defined by file `oi_stack_test/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`oi_stack_test/assets/model-workflow-asset.yml`)
 - batch inference workflow (`oi_stack_test/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`oi_stack_test/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`oi_stack_test/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`oi_stack_test/assets/ml-artifacts-asset.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `oi_stack_test/databricks.yml` and `oi_stack_test/assets/ml-artifacts-asset.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-oi-stack-test-model     | /dev-oi-stack-test-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-oi-stack-test-model | /staging-oi-stack-test-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-oi-stack-test-model    | /prod-oi-stack-test-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-oi-stack-test-model    | /test-oi-stack-test-experiment    |
...
2. Complete the `TODO` in `oi_stack_test/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `oi_stack_test/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `oi_stack_test/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-oi-stack-test-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-oi-stack-test-batch-inference-job`
that runs the notebook under `oi_stack_test/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-oi-stack-test-batch-inference-job`.
...
`test-oi-stack-test-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-oi-stack-test-batch-inference-job`.
